...
class MainWindow(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        for p in range(num_players):
            self.textlbl[p].resize(self.textlbl[p].width(), 50)
            self.textlbl[p].setAutoFillBackground(True)
            self.textlbl[p].setFixedHeight(100)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--first_player", default="abc",
                        help="Player who players first move.")
    ...
    gsfactory = GameServerFactory(mainWin, start_player, num_players, num_moves, gui_on)
    reactor.listenTCP(1337, gsfactory)
    reactor.runReturn()
    sys.exit(app.exec_())
